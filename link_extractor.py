...
def expand_url(url: str, timeout: float = 3.0) -> str:
...
def extract_and_expand_urls(text: str) -> List[str]:
    """Extract all URLs from text and expand short links."""
    urls = extract_urls(text)
    expanded = [expand_url(url) for url in urls]
    return expanded
